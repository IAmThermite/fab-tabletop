...
- A **Grafana Cloud** account (free tier is sufficient) — used for the
  dashboards, Tempo, and database observability.
- A **Sentry** account and project (Elixir platform).
- `flyctl`, authenticated (`fly auth login`).
- For § 4 only: a local `psql`, and Postgres 14+ on the database app (it runs 18).
