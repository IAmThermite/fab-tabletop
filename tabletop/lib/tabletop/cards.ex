    ...
    from(cp in CardPrint,
      where:
        fragment(
          ...
          """,
          # art vs image_phash
          cp.image_phash, type(^art, :integer),
          cp.image_phash, type(^art, :integer), ^art_t,
          # art_flipped vs image_phash
          cp.image_phash, type(^art_flipped, :integer),
          cp.image_phash, type(^art_flipped, :integer), ^art_t,
          # art_left vs image_phash_left
          cp.image_phash_left, type(^art_left, :integer),
          cp.image_phash_left, type(^art_left, :integer), ^art_t,
          # art_left vs image_phash_right
          cp.image_phash_right, type(^art_left, :integer),
          cp.image_phash_right, type(^art_left, :integer), ^art_t,
          # art_right vs image_phash_left
          cp.image_phash_left, type(^art_right, :integer),
          cp.image_phash_left, type(^art_right, :integer), ^art_t,
          # art_right vs image_phash_right
          cp.image_phash_right, type(^art_right, :integer),
          cp.image_phash_right, type(^art_right, :integer), ^art_t,
          # full vs image_phash_full
          cp.image_phash_full, type(^full, :integer),
          cp.image_phash_full, type(^full, :integer), ^full_t
        ),
      order_by:
        fragment(
          """
          ...
          cp.image_phash, type(^art, :integer),
          cp.image_phash, type(^art, :integer), ^art_t,
          cp.image_phash, type(^art, :integer), ^miss,
          # art_flipped
          cp.image_phash, type(^art_flipped, :integer),
          cp.image_phash, type(^art_flipped, :integer), ^art_t,
          cp.image_phash, type(^art_flipped, :integer), ^miss,
          # art_left × image_phash_left
          cp.image_phash_left, type(^art_left, :integer),
          cp.image_phash_left, type(^art_left, :integer), ^art_t,
          cp.image_phash_left, type(^art_left, :integer), ^miss,
          # art_left × image_phash_right
          cp.image_phash_right, type(^art_left, :integer),
          cp.image_phash_right, type(^art_left, :integer), ^art_t,
          cp.image_phash_right, type(^art_left, :integer), ^miss,
          # art_right × image_phash_left
          cp.image_phash_left, type(^art_right, :integer),
          cp.image_phash_left, type(^art_right, :integer), ^art_t,
          cp.image_phash_left, type(^art_right, :integer), ^miss,
          # art_right × image_phash_right
          cp.image_phash_right, type(^art_right, :integer),
          cp.image_phash_right, type(^art_right, :integer), ^art_t,
          cp.image_phash_right, type(^art_right, :integer), ^miss,
          # full
          cp.image_phash_full, type(^full, :integer),
          cp.image_phash_full, type(^full, :integer), ^full_t,
          cp.image_phash_full, type(^full, :integer), ^miss
        ),
      preload: [:card],
      limit: 5
    )
